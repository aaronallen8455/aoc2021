...
day23B (BS.transpose . BS.lines -> lns) =
...
evacRoom :: Path -> Room -> [Path]
evacRoom p@(Path st _) rm = go p (getRoom rm st) where
  go p' xs | all (\x -> getDest x == rm) xs = [p']

  go p' (c:rest) = do
    hi <- hallIx
    Just m <- pure $ numMoves st (Rm rm) (Hall hi)
    let ucost = costToMove c * m
        st' = setRoom rest (pathSt p') rm
        st'' = st' { hall = IM.insert hi (Just c) $ hall st' }
        newPath = Path st'' (pathCost p' + ucost)
    go newPath rest

numMoves :: St -> Loc -> Loc -> Maybe Int
numMoves st (Rm r) dest = do -- traceShow (Rm r, dest) . traceShowId $ do
  let next = numMoves st (Hall $ roomToHall r) dest
  case getRoom r st of
    [] -> Nothing
    xs -> (+ (4 - length xs + 1)) <$> next
numMoves st (Hall a) (Hall b) = -- traceShow (Hall a, Hall b) . traceShowId $
  case compare a b of
    ...
    EQ -> Just 0
    _ -> Nothing
numMoves st (Hall i) (Rm r) = -- traceShow (Hall i, Rm r) . traceShowId $
  if roomToHall r == i
     then case getRoom r st of
            xs | length xs == 4 -> Nothing
